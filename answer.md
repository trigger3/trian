...
题目中出现的漏洞为内存越界。个人猜测publicLength()是求数组元素的个数，而constructEmptyArray()需要的totalsize应该是对应的比特大小。那么此时的totalsize应该为totalsize*array中单个元素的大小。因此result所指向的空间大小便小于了两个数组的大小之和，在拼接两个数组到result中时便出现越界。在编程中尽量减少直接操作内存，而是将操作内存做到足够的封装，而避免每位编程人员都去做内存处理。
